...
#define H_SIZE     1024  /* Size of h[] array - indices in idx[] are in [0, H_SIZE) */
...
int main() {
    ...
    float *h = (float *)malloc(H_SIZE * sizeof(float));
    float *h_copy = (float *)malloc(H_SIZE * sizeof(float));
    float *w = (float *)malloc(ARRAY_SIZE * sizeof(float));
    float *w_copy = (float *)malloc(ARRAY_SIZE * sizeof(float));
    int *idx = (int *)malloc(ARRAY_SIZE * sizeof(int));
    int *idx_copy = (int *)malloc(ARRAY_SIZE * sizeof(int));
    ...
    for (int i = 0; i < H_SIZE; i++) h_copy[i] = h[i];
    for (int i = 0; i < ARRAY_SIZE; i++) {
        w_copy[i] = w[i];
        idx_copy[i] = idx[i];
    }
    ...
    start_timer();
    kernel3_baseline(h, w, idx, ARRAY_SIZE);
    double t_base = stop_timer();

    /* Restore arrays */
    for (int i = 0; i < H_SIZE; i++) h[i] = h_copy[i];
    for (int i = 0; i < ARRAY_SIZE; i++) {
        w[i] = w_copy[i];
        idx[i] = idx_copy[i];
    }
    ...
    Xil_DCacheInvalidate();
    start_timer();
    kernel3_optimized_a(h, w, idx, ARRAY_SIZE);
    ...
    Xil_DCacheInvalidate();
    start_timer();
    kernel3_optimized_b(h, w, idx, ARRAY_SIZE);
