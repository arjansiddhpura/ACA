...
#define ARRAY_SIZE_KERNEL1 4096
...
int run_kernel1_test() {
    ...
    int *A = (int *)malloc(ARRAY_SIZE_KERNEL1 * sizeof(int));
    if (A == NULL) {
    ...
    for (int i = 0; i < ARRAY_SIZE_KERNEL1; i++)
        A[i] = rand() % 1024;

    /* --- Baseline measurement --- */
    Xil_DCacheInvalidate();
    start_timer();
    kernel1_baseline(A, ARRAY_SIZE_KERNEL1, offset);
    ...
    Xil_DCacheInvalidate();
    start_timer();
    kernel1_optimized(A, ARRAY_SIZE_KERNEL1, offset);
