...
#define ARRAY_SIZE_KERNEL4 4096
...
int run_kernel4_test() {
    ...
    float *A = (float *)malloc(ARRAY_SIZE_KERNEL4 * sizeof(float));
    float *B = (float *)malloc(ARRAY_SIZE_KERNEL4 * sizeof(float));
    ...
    for (int i = 0; i < ARRAY_SIZE_KERNEL4; i++) {
    ...
    start_timer();
    volatile float result_base = kernel4_baseline(A, B, ARRAY_SIZE_KERNEL4);
    double t_base = stop_timer();

    /* --- Optimized measurement --- */
    Xil_DCacheInvalidate();
    start_timer();
    volatile float result_opt = kernel4_optimized(A, B, ARRAY_SIZE_KERNEL4);
